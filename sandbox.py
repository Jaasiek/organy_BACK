...
def init_595():
    for _ in range(32):  # zależnie od liczby bitów
        pi.write(SER_1, 0)
        pi.write(SRCLK, 1)
        pi.write(SRCLK, 0)
    pi.write(RCLK, 1)
...
def shift_out(bit_list, pinout):
    ...
    print("shift_out (po reversed):", "".join(str(1 - b) for b in reversed(bit_list)))
    for bit in reversed(bit_list):
        pi.write(pinout, 1 if bit else 0)
        pi.write(SRCLK, 1)
        pi.write(SRCLK, 0)
    pi.write(RCLK, 1)
    pi.write(RCLK, 0)
...
def shift_out_from_cords():
    ...
    print("bit_list ->", "".join(str(b) for b in bit_list))
...
def clear_outputs():
    ...
    print("Wyczyszczono wszystkie wyjścia (cords = 0)")
...
def test_outputs():
    print("Test: po kolei zapalanie 1..32")
    for i in range(1, 33):
        for k in cords:
            cords[k] = 0
        cords[str(i)] = 1
        shift_out_from_cords()
        print(f"LED {i} powinna się świecić")
        time.sleep(0.5)
        time.sleep(1)
    print("Koniec testu")
