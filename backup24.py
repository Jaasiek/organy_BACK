...
# --- GPIO konfiguracja ---
SER_1 = 17  # Wspólny pin danych do rejestrów
SRCLK = 27  # Zegar przesuwający bity (74HC595: SHCP)
RCLK = 22  # Zegar rejestrujący dane (74HC595: STCP)
ENC_CLK = 5
ENC_DT = 6
...
def shift_out_manuals(manual_1, manual_2, pedal):
    full_data = manual_1 + manual_2 + pedal
    assert len(full_data) == 144, f"Expected 144 bits, got {len(full_data)}"
    bits = list(reversed(full_data))
    for bit in bits:
        pi.write(SER_1, bit)
        pi.write(SRCLK, 1)
        pi.write(SRCLK, 0)
